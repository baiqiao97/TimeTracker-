...
> 自动记录你的应用使用时长，按标签分类统计，支持活动模式多场景追踪、目标管理与 AI 辅助分析，数据可通过私有服务器跨设备同步。

## 特性

- ⏱ **自动追踪** — 后台记录每个应用/窗口的使用时长，支持前台/后台识别
- 🏷 **标签分类** — 为应用分配标签（工作/学习/娱乐/社交），内置常见应用自动映射
- 📊 **可视化统计** — 柱状图/饼图/折线图展示每日/每周/每月趋势
- 🎯 **目标管理** — 创建目标、AI 智能拆解阶段、追踪进度与效率评价
- 📋 **任务 & 日程** — 待办事项列表（支持优先级）和日历视图
- 📥 **数据管理** — JSON/CSV 导入导出，定时自动导出，HTML 报告生成
- 🎨 **现代 UI** — 圆角卡片、流畅动画、暗色/亮色主题一键切换
- ⌨️ **全局快捷键** — Ctrl+Shift+T / Ctrl+Shift+P 快捷操作
- 🔒 **私有部署** — 自建服务端，数据不经过第三方，支持数据库加密
